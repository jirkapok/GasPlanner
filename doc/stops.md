...
* `Round deco stops to minutes`: By default stops are calculated in seconds. In case you want them rounded up to minutes, enable this option. Each stop (including safety stop and gas switch) is then prolonged, so the runtime at end of the stop is rounded up to whole minutes. Stops may therefore last less than a minute, because the ascent between stops is not rounded.
