...
export class Consumption {
    ...
    private consumeFromTank(tank: Tank, tankIndex: number, consumedLiters: number): number {
        const consumedLitersOrigin = consumedLiters;
        ...
        consumedLiters = consumedLiters - (tankConsumedBars * tank.size);
        // because of previous rounding up the consumed bars
        consumedLiters = consumedLiters < 0 ? 0 : consumedLiters;

        // TODO remove console.log after consumption is completely implemented
        console.log(`Tank ${tankIndex}: consumedLitersOrigin=${consumedLitersOrigin},
                     consumedBars=${consumedBars}, tankConsumedBars=${tankConsumedBars},
                     tank.consumed=${tank.consumed}, consumedLiters=${consumedLiters};`);
    ...
    public consumedOnWay(segments: Segment[], tank: Tank, sac: number): number {
        const conSegments = ConsumptionSegment.fromSegments(segments);
