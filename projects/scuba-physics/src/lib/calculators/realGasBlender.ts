...
export class RealBlender {
    ...
    public blend(pi: number, gasi: GasMix, pf: number, gasf: GasMix, gas1: GasMix, gas2: GasMix, gas3: GasMix): string {
        if (gasi.fO2) {
            if (gasf.fHe > 0) {
    ...
    private blendTrimix(pi: number, pf: number, gasi: GasMix, gas1: GasMix, gas2: GasMix, gas3: GasMix, gasf: GasMix): string {
        ...
        const top1 =
            ((gas3.fN2 * gas2.fO2 - gas2.fN2 * gas3.fO2) * (gasf.fHe * fvol - gasi.fHe * ivol) +
                (gas2.fHe * gas3.fO2 - gas3.fHe * gas2.fO2) * (gasf.fN2 * fvol - gasi.fN2 * ivol) +
                (gas3.fHe * gas2.fN2 - gas2.fHe * gas3.fN2) * (gasf.fO2 * fvol - gasi.fO2 * ivol)) /
            det;

        const top2 =
            ((gas1.fN2 * gas3.fO2 - gas3.fN2 * gas1.fO2) * (gasf.fHe * fvol - gasi.fHe * ivol) +
                (gas3.fHe * gas1.fO2 - gas1.fHe * gas3.fO2) * (gasf.fN2 * fvol - gasi.fN2 * ivol) +
                (gas1.fHe * gas3.fN2 - gas3.fHe * gas1.fN2) * (gasf.fO2 * fvol - gasi.fO2 * ivol)) /
            det;

        const top3 =
            ((gas2.fN2 * gas1.fO2 - gas1.fN2 * gas2.fO2) * (gasf.fHe * fvol - gasi.fHe * ivol) +
                (gas1.fHe * gas2.fO2 - gas2.fHe * gas1.fO2) * (gasf.fN2 * fvol - gasi.fN2 * ivol) +
                (gas2.fHe * gas1.fN2 - gas1.fHe * gas2.fN2) * (gasf.fO2 * fvol - gasi.fO2 * ivol)) /
            det;
        ...
        const newmix1 = new GasMix(
            (100 * (gasi.fO2 * ivol + gas1.fO2 * top1)) / (ivol + top1),
            (100 * (gasi.fHe * ivol + gas1.fHe * top1)) / (ivol + top1)
        );

        const p1 = this.compress.pressure(newmix1.toGas(), ivol + top1);

        const newmix2 = new GasMix(
            (100 * (gasi.fO2 * ivol + gas1.fO2 * top1 + gas2.fO2 * top2)) / (ivol + top1 + top2),
...
${this.format(top3)} litres of ${gas3.name} per litre of cylinder volume.`;
    ...
    private blendNitrox(pi: number, gasi: GasMix, pf: number, gasf: GasMix, gas1: GasMix, gas2: GasMix): string {
        ...
        const top1 = ((gas2.fO2 - gasf.fO2) / (gas2.fO2 - gas1.fO2)) * fvol - ((gas2.fO2 - gasi.fO2) / (gas2.fO2 - gas1.fO2)) * ivol;
        const top2 = ((gas1.fO2 - gasf.fO2) / (gas1.fO2 - gas2.fO2)) * fvol - ((gas1.fO2 - gasi.fO2) / (gas1.fO2 - gas2.fO2)) * ivol;
